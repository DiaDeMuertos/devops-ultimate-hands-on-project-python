...
def main(urls: list[str], timeout: int, verbose: bool):
    if verbose:
        logging.getLogger().setLevel(logging.DEBUG)
        logger.debug("Verbose loggin enabled.")
    ...
    if not urls:
        ...
        click.echo("Usage: check-urls <URL1> <URL2> ... [--timeout TIMEOUT] [--verbose]")
        return

    logger.info(f"Starting check for {len(urls)} URLs.")

    results = check_url(urls, timeout)
